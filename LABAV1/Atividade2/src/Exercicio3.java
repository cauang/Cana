...
public class Exercicio3 {
	...
	static void metodo1 (long n) {
		...
		for (long i = 10; i < 12; i++) {       //c2     2 + 1
			for (long j = 4; j <= 6; j++) {    //c3     3 * (2 + 1) = 9
				valor += 1;                 //c4     3 * (2) = 6
                //T(n) = O(1) ---- CONSTANTE
				try {
					TimeUnit.MILLISECONDS.sleep(1);
	...
	static void metodo2 (long n) {
		...
		for (long i = 1; i < n; i++) {       //c2  n
			for (long j = 1; j <= 5; j++) {  //c3  n * (6)
				valor += 1;                  //c4  n * (5)  
                //T(n) = O(n) ---- LINEAR
				try {
					TimeUnit.MILLISECONDS.sleep(1);
...
static void metodo3 (long n) {
    long valor = 0;                          // c1   1
    try { TimeUnit.MILLISECONDS.sleep(1); }  // c2   1
    catch (InterruptedException e) { e.printStackTrace(); }

    for (long i = 0; i <= n; i++) {          // c3   testes = (n+1)-0+1 = n+2
                                             //      corpo  = n+1
        for (long j = 1; j <= n - i + 1; j++) { // c4 por i: testes = (n-i+2)
                                                //    soma testes = (n+2)+(n+1)+...+2
                                                //                = (n+1)(n+2)/2
            valor += 1;                          // c5 por i: corpo = (n-i+1)
                                                //    soma corpo = (n+1)+...+1
                                                //               = (n+1)(n+2)/2
                                                // T(n) = γ·(n+1)(n+2)/2 + …  -> Θ(n²) quadrática
...
static void metodo4 (long n) {
    ...
    for (long i = 1; i <= n * n; i++) {      // c3   testes = (n²+1)-1+1 = n²+1
                                             //      corpo  = n²
        for (long j = 0; j <= i; j++) {      // c4 por i: testes = (i+1)-0+1 = i+2
                                             //    soma testes = (i+2), i=1..n²
                                             //                = (n²)(n²+1)/2 + 2n²
            valor += 1;                      // c5 por i: corpo = (i+1)
                                             //    soma corpo = (n²)(n²+1)/2 + n²
            try { TimeUnit.MILLISECONDS.sleep(1); } // mesma soma do corpo
            catch (InterruptedException e) { e.printStackTrace(); }
        }
    }
}
// T(n) = δ·n⁴ + …  -> Θ(n⁴) QUARTICA
